...
typedef struct QUIC_CONN_STATS {
    ...
    struct {
        uint64_t FlushCount;                    // Sum of send flush calls that reached packet builder.
        uint64_t FlushPacingDelayedCount;       // Flushes delayed by pacing allowance.
        uint64_t FlushCcBlockedCount;           // Flushes stopped because congestion control could not send.
        uint64_t FlushSchedulingCount;          // Flushes limited by scheduling batch size.
        uint64_t FlushAmplificationBlockedCount;// Flushes stopped by anti-amplification allowance.
        uint64_t FlushNoWorkCount;              // Flushes that found no frames/streams to send.
        uint32_t LastFlushAllowance;            // Last packet builder send allowance.
        uint32_t LastFlushPathAllowance;        // Last path anti-amplification allowance.
        uint32_t LastFlushResult;               // Last QUIC_SEND_RESULT value.
        uint32_t LastFlushDatagrams;            // Datagrams generated by the last flush.
        uint32_t LastOutFlowBlockedReasons;     // Last connection OutFlowBlockedReasons value.
